...
b. Dibujar en una gráfica las 50 primeras sumas parciales de la serie.
...
b. Dibujar en una misma gráfica la suma funcional parcial de orden 10 de la serie y la función de Bessel de orden 0.
...
c. $\sum \dfrac{\operatorname{sen}((2n-1)t)}{2n-1}$
...
¿Hasta qué suma parcial de la serie de Maclaurin de la función $e^x$ hay que llegar para aproximar el número $e$ con un error menor de $10^-10$?
...
donde R es el radio de la Tierra (tomar un radio medio 6371 km) y $g$ es la aceleración de la gravedad (tomar $9.81$ m/s^2). Expresar $F$ como una serie de potencias de $h/R$ y aplicar el teorema de la serie alternada para calcular los valores de $h$ para los que la aproximación clásica $F=mg$ no difiere más de un 1% del valor real.

```{julia}
#| hold: true
#| echo: false
using SymPy
@vars x m g real=true
a(n) = m*g*(n+1)*(-x)^n
a(0)
numericq(1000, 1)
```
