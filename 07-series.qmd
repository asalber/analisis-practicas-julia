...
Usar las sumas parciales hasta orden 100 de las siguientes series para calcular de manera aproximada el valor de $\pi$. ¿Qué serie converge más rápidamente?
...
a.  $\sum \frac{1}{n^2}$
    ...
    @syms x::real
    a(n) = 1/n^2
    An = cumsum([a(i) for i=1:100])
    n = [10*i for i = 1:10]
    for i in n
        error = float(integrate(a(x), (x, i, oo)) - integrate(a(x), (x, i+1, oo))) 
        println("Aproximación de π con $i términos: $(sqrt(6*An[i])), error: $error")
    end
    ```
    :::
:::
