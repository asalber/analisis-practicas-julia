...
También es posible dibujarlo de manera interactiva con la activando el backend de [Plotly](https://plotly.com/julia/) mediante la función [`plotlyjs()`](https://docs.juliaplots.org/latest/gallery/plotlyjs/).

```{julia}
# Activamos el backend de Plotly.
plotlyjs()
plot(surface(ws...))
```
...
δ = 900
gravedad = 9.81
N(integrate(gravedad*δ*PI*f⁻¹(y)^2*(8-y), 0, nivel))
```
:::
...
f(x) = sin(x)
g(x) = cos(x)
sol = N(integrate(abs(f(x)-g(x)), 0, 2pi))
numericq(sol, 1e-5, hint="Introducir hasta 5 decimales")
```
:::
