...
a.  Dibujar la gráfica y el punto $(1, 1, f(1,1))$.
    ...
    Usar la función [`scatter`](https://aprendeconalf.es/manual-julia/graficos.html#a%C3%B1adir-puntos-a-una-gr%C3%A1fica) del paquete `Plots` o la función [`scatter`](https://aprendeconalf.es/manual-julia/graficos.html#diagrama-de-puntos) del paquete Makie para dibujar puntos.
    :::
    ...
    f(x, y) = x^2 - 2y^2
    xs = ys = range(-2, 2, 50)
    Plots.surface(xs, ys, f, xlab = "x", ylab = "y", zlab = "z", camera=(45,30))
    Plots.scatter!([1], [1], [f(1,1)], color=:red)
    ```
...
a.  Dibujar el plano $x=1$ sobre la superficie. ¿Cuál es la curva que resulta de la intersección del plano con la superficie?
