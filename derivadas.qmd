...
c. Calcular la velocidad media (tasa de variación media) desde el instante $t=3$ hasta los instantes $t=6, 5, 4$ y dibujar la rectas secantes a la gráfica anterior en esos instantes. ¿Cómo es la tendencia de las velocidades medias? ¿Hacia dónde tiende la velocidad media cuando los aproximamos al instante $t=3$ con instantes menores y mayores?
...
a. Dibujar la gráfica de $f$ y dibujar las rectas secantes a $f$ en los intervalos $[\frac{i}{10}, 1]$ para $i=0,\ldots, 9$. ¿Hacia dónde tienen las pendientes de las rectas secantes?
...
:::{.callout-note collapse="true"}
## Ayuda
Usar la función [`diff`](https://docs.juliahub.com/SymPy/KzewI/1.0.31/Tutorial/calculus/#Derivatives-1) del paquete `SymPy` para calcular simbólicamente la derivada.
:::
...
@vars x real=true
h(x) = sin(x)+cos(x)
println("Primera derivada: ", diff(h(x),x))
...
## Solución
```{julia}
plot!(diff(g(x),x,2), label=L"g''(x)="*latexify(simplify(diff(g(x),x,2))))
```
:::
...
## Solución
```{julia}
using ImplicitPlots, Plots, SymPy, LaTeXStrings
@vars x y real=true
f(x,y)=x^4-3x^2+2y^2
...
println("Aproximación de ln(1.2): ", float(pol(0.2)))
```
...
```{julia}
println("Cota del error: " , maximum(float.(abs.([resto(0), resto(0.2)])))) 
