...
:::{.callout-note collapse="true"}
## Pista
Definir una función para el término general y aplicar la función a los naturales de 1 a 50 usando compresiones de arrays como en el ejercicio anterior. Después usar la función [`scatter`](https://docs.juliaplots.org/latest/api/#Plots.scatter-Tuple) del paquete `Plots` para dibujar el array de términos.
:::
...
e. $x_1 = 2$ y $x_{n+1}=1+\frac{1}{x_n}$ $\forall n\in\mathbb{N}$
...
¿Cuál de las siguientes gráficas corresponde a la sucesión $x_1=3$ y $x_{n+1}=\sqrt{2x_n}$ $\forall n=2,3,\ldots$.
```{julia}
#| echo: false
using Plots
